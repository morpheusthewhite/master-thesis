...
\begin{proof}
	...
	\begin{proof}
		...
		By construction $m_{2}^{+} = n_{1} $ and $m_{2}^{-} \geq \lambda n$.
		Consequently
...
\begin{theorem}
	...
	Densest Echo Chamber Problem (DECP) has no $n^{1-\epsilon} $-approximation algorithm for
	any $\epsilon$ unless $\mathcal{P} = \mathcal{NP}  $
\end{theorem}
...
\begin{proof}
	...
	\begin{proof}
		...
		\begin{equation}
			OPT(ECP) \geq \psi(U) = |I| \implies OPT(DECP) \geq OPT(MIS)
		\end{equation}
