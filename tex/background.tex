...
objective function cannot be decreased and we found an optimum. If, instead, there is at least a $c_{j}
	< 0$ then we can decrease the value of $\zeta$ by increasing $x_{j} $; now
...
\begin{itemize}
	...
	\item if no such $\tilde{x}_{j} $ variable exists then the value of $x_{j}
	      ...
	      \emph{unbounded}, i.e. it can achieve any arbitrarily small value.
\end{itemize}

The process is illustrated in picture \autoref{fig:tex/img/simplex}
