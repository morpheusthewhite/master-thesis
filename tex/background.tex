...
will initially discuss graph density problems (\autoref{sec:signed_graphs_and_density}) which are significant in the
...
different definitions of density have been used, each corresponding to
different problems
...
Then it returns the $S$ which, during the process, was associated with the
highest density $f(S)$.
...
According to the choice of the aggregate function there are different variants
of the problem
...
\begin{itemize}
	...
	\item \acrshort{DCS}-AM, whose density aggregation function is
	      ...
	      generally (i.e. in at least some graphs in the sequence)
	...
	\item \acrshort{DCS}-AA maximizing
	      ...
	      which puts fewer restrictions that the previous variants on the
	      ...
	      More specifically, solving this problem is the equivalent of solving
...
The algorithm starts by solving the \acrshort{BFF} problem on each of the
...
\footnotetext{Given $2$ sets $C_1$ and $C_2$ the Jaccard coefficient is equal
...
computational tasks; more specifically it aims at determining the
...
Programming Models \cite{KHACHIYAN198053}\cite{Karmarkar1984} and finding whether
...
According to these definitions it is easy to see that $\mathcal{P} \subseteq
...
\begin{figure}[]
	...
	\caption[$\mathcal{NP}$-Hardness in post-covid times]{\emph{As if these problems weren't $\mathcal{NP} $-Hard enough}.
...
\label{par:_np_hard} is the set of $\mathcal{NP} $-Hard problems that are
also in $\mathcal{NP} $. Intuitively these correspond to the most difficult
...
If, as it is mostly accepted, $\mathcal{P} \neq \mathcal{NP} $ then we have
...
\emph{Approximation preserving reduction} must have
...
\emph{nonbasic variables} \cite{Vanderbei2008}\cite{Bertsimas1997} and
...
for getting an approximation of the exact solution and it is much easier
\cite{Conforti2016}.
...
\begin{itemize}
	\item If either of the subproblems is not feasible or its value $z_{j} $ is
	      lower than the best one found so far then it does not to be
...
When there are no remaining subproblems to consider then Branch-and-Bound method
