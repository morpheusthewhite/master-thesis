...
Let $G$ be our \emph{Interaction Graph} for contents $\mathcal{C} $ and
threads $T \in \mathcal{T}_{C}, \; C \in \mathcal{C} $. Fix
...
and $E^{-}_k $ be the set its positive and negative edges, respectively.
...
\begin{proof}
	...
	\begin{proof}
		...
		Let us now consider a thread $T_k \in \mathcal{S}(U)$; this means that
		...
		if $T_k \not\in \mathcal{S}(U)$ then $x_{ij}^{k} = 0 \; \forall ij \in E_{k}$ and
	...
	\begin{proof}
		...
		Let us know consider $T_k \; s.t. \; z_k = 1$. Because of
		\autoref{eq:ecp-alpha-constraint1} and \autoref{eq:ecpp-z-iff-xij} we have
		...
		Now suppose $\exists \; T_k \in \mathcal{S}(U) \; s.t.
		...
		\begin{gather*}
			...
			0.5 \cdot |E^{+}_{k}[U]| - 0.5 \cdot |E^{-}_{k}[U]| \geq 0 \\
			|E^{+}_{k}[U]| - |E^{-}_{k}[U]| \geq 0
		\end{gather*}
		Consequently $T_k$ will contribute positely to $\xi(U)$.

		Also, any $T_k$ for which $0 < z_k < 1$ can be similarly shown to be
...
\footnotetext{\label{fn:epsilon-constraint}A strict inequality is not allowed by the definition of
	...
	\end{equation*}
	We will however use \autoref{eq:ecp-a-alpha-constraint} for simplifying notation
}
...
\begin{proof}
	...
	\begin{proof}
		...
		Equation~\ref{eq:d-ecp-vertex-l-b} is trivial and
		...
		Thus, the second inequality of \autoref{eq:d-ecp-alpha-constraint} is true;
